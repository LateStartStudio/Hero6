...
namespace LateStartStudio.Hero6.ModuleController.Campaigns.Rooms.Regions
{
    ...
    public class WalkArea
    {
        ...
        public WalkArea(int width, int height, IDictionary<int, Node> nodes)
        {
            ...
            Height = height;
            Nodes = nodes;
            pathfinder = new AStar(OpenSetCapacity, FindNeighborDistance, CalculateOctileHeuristic);
        }
        ...
        {
            var nodes = pathfinder.FindPath(Nodes[(from.Y * Width) + from.X], Nodes[(to.Y * Width) + to.X]);
        ...
        {
            var fromX = from.ID % Width;
            var fromY = from.ID / Width;
            var toX = to.ID % Width;
            var toY = to.ID / Width;
